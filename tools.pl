...
#$rcs = ' $Id: tools.pl,v 1.18 2004/11/12 20:25:00 daniel Exp $ ' ;
...
sub isHTML {
  ...
  if( $ext ) {
    return grep(/^\Q$ext\E$/i, @HIGHLIGHT_EXT);
  } else {
    return 0;
  }
}
...
sub get_url {
  ...
  $url = $response->base;
  my $date_last_modified = $response->last_modified;
  return ($url, $date_last_modified, $size, $buffer);
}
